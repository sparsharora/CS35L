...
scene_t scene;
int nthreads;
float finalp[width][height][3];

//declared some variables globally so that I could use them in my own function for multithreading

void* multithreadedf(void* tnum)
{
  ...
  const double pixel_dy = tan( 0.5*camera_fov ) / ((double)height*0.5);
    ...
    /* for every pixel */
    for( int px=x ; px<width ; px+=nthreads)
      {
        ...
        for( int py=0; py<height; ++py )
	  {
	    ...
	    // float scaled_color[3];
            //scaled_color[0]
	    finalp[px][py][0]= gamma( pixel_color[0] ) * max_color;
	    finalp[px][py][1]= gamma( pixel_color[1] ) * max_color;
	    finalp[px][py][2]= gamma( pixel_color[2] ) * max_color;            
	    //scaled_color[1] = gamma( pixel_color[1] ) * max_color;
            ...
            for( int i=0; i<3; i++)
	      finalp[px][py][i] = max( min(finalp[px][py][i], 255), 0);
	  }
      }
    return NULL;
}
...
main( int argc, char **argv )
{
    ...
    for(;i<nthreads;i++)
      {
	arr[i]=i;
	int ret = pthread_create(&tids[i], NULL, multithreadedf, &arr[i]);
	if(ret)
	  {
	...
	for(j=0; j < height ; j++)
	{
             ...
             */
            printf( "%.0f %.0f %.0f\n",
		    finalp[i][j][0], finalp[i][j][1], finalp[i][j][2] );
        }
        printf( "\n" );
      }
